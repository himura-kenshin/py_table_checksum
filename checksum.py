        ...
        for key in range(1, maxid, 10000):
            ...
            try:
                ...
                try:
                    ...
                    chunk_time = round(
                        endtime.timestamp() - starttime.timestamp(), 6)
            # 获取本批次crc值和行数值
...
def target(host, port, username, password, dbname):
    ...
    sql = "select date_format(max(ts),'%Y-%m-%d %H:%I:%S') as TS, " + str(len(diffs)) + \
        " as DIFFS,cast(sum(master_cnt) AS   CHAR) as ROWS, max(chunk) as CHUNKS,round(sum(chunk_time),3) as TIME,concat(db,'.',tbl) as  'TABLE'  from checksums"
    cursor.execute(sql)
...
def do(argv):
    ...
    result, diffs = target(
        argv['s_host'], argv['s_port'], argv['s_user'], argv['s_password'], "percona")
    print(result, diffs)
    return result, diffs
